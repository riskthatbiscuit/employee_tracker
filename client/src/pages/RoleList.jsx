        ...
        <form onSubmit={handleSubmit(handleAddRole)} className="mb-4">
          ...
          >
            {departments.map((department, index) => (
              <option key={`${department.department_name}-${index}`} value={department.department_id}>
                {department.department_name}
              </option>
            ))}
          </select>
          </div>
        <button type="submit" className="p-2 bg-blue-500 text-white rounded">Add Role</button>
      </form>
      <ul className="space-y-4">
        {roles.map((role) => (
          <li key={role.id} className="p-4 border rounded-lg shadow-md">
            <p className="text-lg font-semibold"><strong>Title:</strong> {role.job_title}</p>
            <p className="text-gray-700"><strong>Salary:</strong> {role.salary}</p>
            <p className="text-gray-700"><strong>Department:</strong> {role.department}</p>
            <button
